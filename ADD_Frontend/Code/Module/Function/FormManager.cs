...
namespace ADD_Frontend.Code.Module.Function
{
    ...
    internal static class FormManager
    {
        ...
        private static void SaveParams(Form f)
        {
            if(f is OGenericDay ogd)
            {
                Prop.Default.DAY = ogd.CADay.Value;
                OGenericParams(ogd);
            } 
            else if (f is OGenericMonth ogm)
            {
                Prop.Default.MONTH = ogm.MPMonth.Value;
                OGenericParams(ogm);
            }
            else if (f is OGenericRange ogr)
            {
                Prop.Default.START = ogr.CAStart.Value;
                Prop.Default.END = ogr.CAEnd.Value;
                OGenericParams(ogr);
            }
            else if (f is EGenericMonth egm)
            {
        ...
        private static void ManageDayOMIE(Form f)
        {
            ...
            // Init input
            InputOMIE inp = new(sgf.FID, sgf.Day, sgf.LBFolder.Text, sgf.CBKeep.Checked, sgf.CBProcess.Checked, sgf.LBFile.Text);
            // Download file
        ...
        private static void ManageRangeOMIE(Form f)
        {
            ...
            // Init input
            InputOMIE inp = new(mgf.FID, mgf.Start, mgf.End, mgf.LBFolder.Text, mgf.CBKeep.Checked, mgf.CBProcess.Checked, mgf.LBFile.Text);
            // Download files
            DL_OMIE.ManageDL(inp);
            // Close form
            f.Close();
        ...
        private static void ManageMonthOMIE(Form f)
        {
            ...
            HMT hmt = (HMT)f;
            // Init input
            InputOMIE inp = new(hmt.FID, hmt.MPMonth, hmt.LBFolder.Text, hmt.CBKeep.Checked, hmt.CBProcess.Checked, hmt.LBFile.Text);
            // Download files
            DL_OMIE.ManageDL(inp);
            // Close form
            f.Close();
        ...
        private static void ManageMonthESIOS(Form f)
        {
            ...
            C2L c2l = (C2L)f;
            // Init input
            InputESIOS inp = new(c2l.FID, c2l.Month, c2l.LBFolder.Text, c2l.CBKeep.Checked, c2l.CBProcess.Checked, c2l.CBUnzip.Checked, c2l.LBFile.Text);
            // Download files
        ...
        internal static void DownloadDir(FolderBrowserDialog fb_directory, Label2 lb_download, ToolTip tt_folder)
        {
            ...
            fb_directory.InitialDirectory = ADD_Frontend.Properties.Settings.Default.DIRECTORY;
            if (fb_directory.ShowDialog() == DialogResult.OK)
            {
                ...
                ADD_Frontend.Properties.Settings.Default.DIRECTORY = fb_directory.SelectedPath;
            }
        }
        ...
        {
            sf_file.InitialDirectory = ADD_Frontend.Properties.Settings.Default.DIRECTORY;
